...
class BrowserManager:
    ...
    async def screenshot(self, path="screenshot.png"):
        ...
        if not self.page:
            return "Browser failed to start."
        try:
            safe_path = self._safe_screenshot_path(path)
            if safe_path is None:
                return "Error taking screenshot: path must be relative and cannot use '..'"
            await self.page.screenshot(path=safe_path)
            return f"Screenshot saved to {safe_path}"
        except Exception as e:
            return f"Error taking screenshot: {str(e)}"

    @staticmethod
    def _safe_screenshot_path(path: str):
        p = Path(str(path))
        if p.is_absolute():
            return None
