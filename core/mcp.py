...
class MCPClient:
    ...
    async def connect(self):
        ...
        try:
            ...
            if not cmd:
                raise Exception("Missing MCP command")
            cmd_base = Path(cmd).name
            if Config.HADOUKING_ALLOWED_MCP_COMMANDS and cmd_base not in Config.HADOUKING_ALLOWED_MCP_COMMANDS:
                raise Exception(
                    f"MCP command '{cmd_base}' not allowed. "
                    f"Allowed: {sorted(Config.HADOUKING_ALLOWED_MCP_COMMANDS)}"
                )
